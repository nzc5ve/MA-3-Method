...
########
    ...
    c = np.copy(X)
    for i_f in range(X_index):
        X_temp = np.copy(c)
        c[:,:,X_index-i_f] = X[0,1,-1]
        Y_temp = base_model.predict(c)
        if sum((max(Y_temp[0]) == Y_temp[0]) * Y[0]) == 0:
            break
        elif i_f == X_index-3:
            break

    c = np.copy(X_temp)
    for j_b in range(X_index):
        X_temp = np.copy(c)
        c[:,:,j_b] = X[0,1,-1]
        Y_temp = base_model.predict(c)
        if sum((max(Y_temp[0]) == Y_temp[0]) * Y[0]) == 0:
            break
        elif j_b >= X_index - i_f -3:
            break

    print('significant part start at',j_b,'end at',X_index-i_f,'misalignment percentage is',(i_f+j_b)/X_index)
...
########
    ...
    for i_f in range(a_index):
        ...
        Y_test1 = base_model.predict(c)
        if sum((max(Y_test1[0]) == Y_test1[0]) * Y_cur[0]) == 0:
            break
...
print("Success Rate:", (1 - (fail_count/X_train.shape[0])))
