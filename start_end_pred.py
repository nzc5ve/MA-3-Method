...
for position in range(184):
    ...
    move_f = min(move_f)
    move_b = min(move_b)
    sum_align += move_f+move_b+1

print("number of aligned time stamps is", sum_align/184)
'''
move_f = 0
move_b = 1
...
########
    ...
    if sum((max(Y_pred[0]) == Y_pred[0]) * Y[0]) == 0:
        ...
        fail_count += 1
        continue
#print('number of input that is still classified correctly', 1842-invalid_count-fail_count)
#print('fail count is', fail_count)
#print('invalid count is', invalid_count)
#print('success ratio is', (1842-fail_count-invalid_count)/(1842-invalid_count))
#print('consider the invalid success ratio is', (1842-fail_count-invalid_count)/1842)
...
print("Success Rate With Start-End Effect", (success_with_se/(X_train.shape[0])))
