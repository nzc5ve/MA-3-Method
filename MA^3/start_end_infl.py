...
########
    a = X_train[i-1:i]
    b = X_train[i+1:i+2]
    a_index = max(225-len(np.where(a[0,1] == a[0,1,-1])[0])-1, 3) 
    X_index = max(225-len(np.where(X[0,1] == X[0,1,-1])[0])-1, 3)
    b_index = max(225-len(np.where(b[0,1] == b[0,1,-1])[0])-1, 3)
    Y_cur = base_model.predict(X)
    c = np.copy(X)
    for i_f in range(a_index):
        X_true = np.copy(c)
        c = np.append(a[:,:,a_index-i_f:a_index-i_f+1],c,axis=-1)
        c = np.append(c[:,:,:X_index+1],c[:,:,X_index+2:],axis=-1)
        Y_temp = base_model.predict(c)
        if sum((max(Y_temp[0]) == Y_temp[0]) * Y_cur[0]) == 0:
            break
        elif i_f == int((X_index+1)/2):
            break

    c = np.copy(X)
    for j_b in range(b_index):
        #X_true = np.copy(c)
        ...
        if sum((max(Y_temp[0]) == Y_temp[0]) * Y_cur[0]) == 0:
            break
        elif j_b >= min(X_index - i_f -3, int((X_index+1)/2)):
            break

    influential_part = X[:,:,j_b:X_index-i_f+1]
    print('influential portion start at',j_b,'end at',X_index-i_f,'misalignment percentage is',(i_f+j_b)/X_index)
    #X_temp = np.append(np.array([[[X[0,1,-1]]*(X_index+1-len(influential_part[0,1]))]*2]), influential_part, axis=-1)
    #X_temp = np.append(X_temp, np.array([[[X[0,1,-1]]*(224-X_index)]*2]), axis=-1)
    X_temp = np.append(np.array([[[X[0,1,-1]]*j_b]*2]), influential_part, axis=-1)
    X_temp = np.append(X_temp, np.array([[[X[0,1,-1]]*(225-X_index+i_f-1)]*2]), axis=-1)
    
    y = base_model.predict(X_temp)
    Y_temp = np.array([(max(y[0])==y[0])*np.array([1]*len(y[0]))])
    
    var_true = np.zeros((1,128))
########
    ...
    while(query < Max_Query_count):
        ...
        '''
########
        var = np.append(np.array([[0]*int((j_b+1)*128/X_index)]*var.shape[0]), var[:,int((j_b+1)*128/X_index):128-int((i_f+1)*128/X_index)],axis=-1)
        var = np.append(var, np.array([[0]*(128-var[0].shape[0])]*var.shape[0]),axis=-1)
########
        '''
        l0_loss, l2_loss, losses, scores = sess_autoZoom.run([Loss, Dist, f, t], feed_dict={latent_adv: var, x0: X_temp, t0: Y_temp, 
        ...
        if(sum((scores[0] == max(scores[0]))*Y_temp[0])==0 and l2_loss < best_l2):
...
########
            ...
            var_true = np.append(np.array([[0]*int((i_f+1)*128/X_index)]*var_true.shape[0]), var_true[:,:128-int((i_f+1)*128/X_index)],axis=-1)
            l0_loss, l2_loss, losses, scores = sess_autoZoom.run([Loss, Dist, f, t], feed_dict={latent_adv: var_true, x0: X_true, t0: Y,
                                                                            CONST_LAMBDA: init_lambda})
...
########
        ...
        if(query >= Max_Query_count and not np.math.isinf(best_l2)):
            ...
            ########
            #var_true = np.append(var_true[:,int((j_b+1)*128/X_index):], np.array([[0]*int((j_b+1)*128/X_index)]*var_true.shape[0]),axis=-1)
            var_true = np.append(np.array([[0]*int((i_f+1)*128/X_index)]*var_true.shape[0]), var_true[:,:128-int((i_f+1)*128/X_index)],axis=-1)
            l0_loss, l2_loss, losses, scores = sess_autoZoom.run([Loss, Dist, f, t], feed_dict={latent_adv: var_true, x0: X_true, t0: Y,
                                                                            CONST_LAMBDA: init_lambda})
            if(sum((scores[0] == max(scores[0]))*Y[0])==0):
...
print("Success Rate With Start-End Effect", ((success_with_se)/(X_train.shape[0]-invalid_count)))
print("Average time used", time_total/(success_with_se))
...
#        a.append(sum(sum(abs(summary['adv'][i]) > 0)) /2/ max(sum(abs(X_test[i][0])>0), sum(abs(X_test[i][1])>0), 1))
#
#avg_l0 = sum(a) / len(a)
